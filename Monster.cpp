...
Monster::Monster(std::vector<QPointF> path):Enemy(path)
{
    this->waypoints=path;
    this->hp=250.0;
    this->speed=1.0;
    ...
    if (!m_monsterTexture.load(":/assets/monster.png")) {
        qDebug() << "Failed to load monster texture!";
    } else {
        m_monsterTexture = m_monsterTexture.scaled(35, 35, Qt::KeepAspectRatio, Qt::SmoothTransformation);
    }
}
...
void Monster::showhp(QPainter *painter)
{
    // 血条总宽度
...
void Monster::move(int row,int col)
{
