...
Tank::Tank(std::vector<QPointF> path):Enemy(path)
{
    this->waypoints=path;
    this->hp=500.0;
    this->speed=0.5;
    ...
    QImage img(":/assets/tank.png");
    if (img.isNull()) qDebug() << "Failed to load tank texture!";
...
void Tank::showhp(QPainter *painter)
{
    // 血条总宽度
...
void Tank::move(int row,int col)
{
